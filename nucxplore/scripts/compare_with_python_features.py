#!/usr/bin/env python3
"""
Compare Rust nucxplore output against pre-generated Python feature CSVs.

This script does NOT run the original Python pipeline. It only:
1) loads image + instance-map (.mat),
2) runs nucxplore extraction (direct array API or file-based API),
...
def get_nuxplore_module():
    ...
    if _NF is None:
        try:
            import nucxplore as nf
        except ImportError as exc:  # pragma: no cover
            raise SystemExit(
                "nucxplore is not importable. Install the wheel/env first."
            ) from exc
        _NF = nf
    return _NF
...
def get_io_module():
    ...
    if _IO is None:
        try:
            import nucxplore.io as io_mod
        except ImportError as exc:  # pragma: no cover
            raise SystemExit(
                "nucxplore.io is not importable. Install or upgrade the nucxplore wheel."
            ) from exc
        _IO = io_mod
    return _IO
...
def rust_extract_features(
    ...
    if extractor_api == "files":
        ...
        if not hasattr(nf, "extract_features_from_files"):
            raise RuntimeError(
                "nucxplore.extract_features_from_files is unavailable; install an updated package."
            )
