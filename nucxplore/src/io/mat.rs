...
fn parse_element(bytes: &[u8], endian: Endian) -> Result<Element<'_>> {
    ...
    let small = is_small_element(tag_word, endian);
    let (data_type, payload_size, header_size) = if small {
    ...
    let total_size = if small {
        // Small data element: 4-byte tag + up to 4 bytes payload, aligned to 8 bytes.
        8usize
    } else {
...
mod tests {
    ...
    fn write_temp_mat(arrays: &[Vec<u8>]) -> PathBuf {
        ...
        let stamp = SystemTime::now()
            ...
            .as_nanos();
        let path = std::env::temp_dir().join(format!("nucxplore_test_{stamp}.mat"));
    ...
    fn make_small_element_matrix(name: &str, rows: i32, cols: i32, values: &[i32]) -> Vec<u8> {
        // Like make_numeric_matrix but uses small-element tag for the variable name.
        // Small-element tags are 4-byte headers with the size packed in the upper 16 bits.
        assert!(
            name.len() <= 4,
            "name must be <=4 bytes for small element test"
        );
    ...
    #[test]
    fn test_small_element_matrix_name() {
        // A matrix whose variable name is short enough for small-element encoding.
        let arr = make_small_element_matrix("m", 2, 2, &[10, 20, 30, 40]);
        let path = write_temp_mat(&[arr]);

        let (map, key) = load_instance_map(&path, Some("m")).expect("load small-name map");
        assert_eq!(key, "m");
        assert_eq!(map.shape(), &[2, 2]);
        assert_eq!(map[[0, 0]], 10);
        assert_eq!(map[[1, 0]], 20);
        assert_eq!(map[[0, 1]], 30);
        assert_eq!(map[[1, 1]], 40);

        let _ = std::fs::remove_file(path);
    }

    #[test]
    fn test_small_element_auto_detect_key() {
        let arr = make_small_element_matrix("v", 1, 3, &[7, 8, 9]);
        let path = write_temp_mat(&[arr]);

        let (map, key) = load_instance_map(&path, None).expect("load auto map");
        assert_eq!(key, "v");
        assert_eq!(map.shape(), &[1, 3]);
        assert_eq!(map[[0, 0]], 7);
        assert_eq!(map[[0, 1]], 8);
        assert_eq!(map[[0, 2]], 9);
