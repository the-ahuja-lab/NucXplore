...
- [x] 2026-05-02T15:21Z — Implemented `bin/cell_type_predict.py` with strict missing-feature failure and JSON/CSV manifests.
- [x] 2026-05-02T15:21Z — Added `tests/test_cell_type_predict.py` unit tests for success and missing-feature failure paths.
- [x] 2026-05-02T15:21Z — Validated `py_compile` and CLI help; full test execution blocked by missing local `pytest`/`pandas`.
- [x] 2026-05-02T15:30Z — Added `nextflow.config` and `conf/docker.config` to formalize DockerHub image contract and profile behavior.
- [x] 2026-05-02T15:30Z — Updated usage docs with Nextflow config contract and container override pattern.
- [x] 2026-05-02T15:30Z — Validated config parsing and confirmed expected fail-fast behavior for placeholder `params.container`.
- [x] 2026-05-02T16:08Z — Implemented `main.nf` with `roots` and `samplesheet` modes plus `PREPARE_SAMPLESHEET`, `EXTRACT_FEATURES`, and `PREDICT_CELL_TYPES` processes.
- [x] 2026-05-02T16:08Z — Implemented `bin/samplesheet_to_pairs.py` for strict samplesheet validation and mirrored staging.
- [x] 2026-05-02T16:08Z — Validated workflow structure with `nextflow run main.nf -stub-run` in both input modes.
- [x] 2026-05-02T16:12Z — Added fixture data (tiny PNG/MAT pair + samplesheet) under `tests/fixtures/celltype/`.
- [x] 2026-05-02T16:12Z — Added Milestone 5 smoke checks: `tests/run_stub_pipeline_checks.sh` and `tests/test_pipeline_contract.py`.
- [x] 2026-05-02T16:12Z — Validated Milestone 5 checks in `nuxplore_test` via micromamba only.
- [x] 2026-05-02T16:15Z — Expanded `README.md` and `docs/usage.md` with quickstart, parameter reference, and troubleshooting.
- [x] 2026-05-02T16:15Z — Added `CHANGELOG.md` entry for workflow, validation, and docs milestones.
- [x] 2026-05-02T16:15Z — Re-validated contract smoke test wrapper in `nuxplore_test` after docs/release updates.
- [x] 2026-05-02T16:39Z — Added local `Dockerfile` + `.dockerignore` for building runtime image with NuXplore, predictor CLI, and baked models.
- [x] 2026-05-02T16:39Z — Ran non-stub pipeline in both `roots` and `samplesheet` modes against local image `nuxplore-celltype:local`.
- [x] 2026-05-02T16:39Z — Fixed non-stub issues: extraction invocation path, samplesheet relative path resolution, and stable publish behavior.
